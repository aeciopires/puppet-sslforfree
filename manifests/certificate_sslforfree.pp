...
#
# Actions:
#   Instala e gerencia o certificado generado pelo site
#     https://www.sslforfree.com. Tambem cria o certificado no formato JKS.
#   Install and manage the certificate generated by the site
#     https://www.sslforfree.com. Also creates the certificate in JKS format.
#
# Sample Usage:
#
...
class puppet_sslforfree::certificate_sslforfree(
  ...
  if $overwrite_certificate {
    exec { 'remove_certificate_keystore_jks':
      ...
      onlyif   => "test $(${keytool} -list -keystore ${keystore_file} \
    ...
    exec { 'remove_ca_cacerts_jks':
      command  => "true; \
        ${keytool} -delete -keystore ${cacerts_file} -alias ${ca_cert_alias} \
          -storepass ${keystore_pass} -noprompt; ",
      onlyif   => "test $(${keytool} -list -keystore ${cacerts_file} \
        -alias ${ca_cert_alias} -v -storepass '${keystore_pass}' \
        | grep Exception | wc -l) -gt 0",
      provider => 'shell',
      path     => ['/usr/local/sbin', '/usr/local/bin','/usr/sbin','/usr/bin',
        '/sbin','/bin'],
      timeout  => '14400', #equivalente a 4 horas
    ...
    exec { 'remove_ca_cacerts_java':
      ...
      onlyif   => "test $(${keytool} -list -keystore ${java_cacert} \
    ...
    exec { 'certificate_jks':
      command  => "true; \
        ...
        ${keytool} -import -v -trustcacerts -alias ${ca_cert_alias} \
          ...
          -storepass '${keystore_pass}' -noprompt; \
        cd - ; ",
      onlyif   => "test 0 -lt $(${keytool} -list -keystore ${keystore_file} \
        -alias ${cert_alias} -v -storepass '${keystore_pass}' | grep Exception \
        | wc -l)",
      provider => 'shell',
      path     => ['/usr/local/sbin', '/usr/local/bin','/usr/sbin','/usr/bin',
    ...
    exec { 'add_certificate_in_java':
      command  => "true; \
        ...
        ${keytool} -import -v -trustcacerts -alias ${cert_alias} \
          ...
          -storepass '${keystore_pass_default}' -noprompt; \
        cd -; ",
      onlyif   => "test 0 -lt $(${keytool} -list -keystore ${java_cacert} \
        -alias ${ca_cert_alias} -v -storepass '${keystore_pass_default}' | \
        grep Exception | wc -l)",
      provider => 'shell',
      path     => ['/usr/local/sbin', '/usr/local/bin','/usr/sbin','/usr/bin',
